...
def control_default(msg, args):
	...
	#The model_states contains the position, orientation, velocities of all objects in gazebo. In the simulation, there are objects like: ground, Contruction_cone, quadcopter (named as 'default') etc. So 'msg.pose[ind]' will access the 'default' object's pose information i.e the quadcopter's pose.
	ind = msg.name.index('default')
...
velPub = rospy.Publisher('/default/joint_motor_controller/command', Float64MultiArray, queue_size=4)

#Subscribe to /gazebo/model_states to obtain the pose in quaternion form
#Upon receiveing the messages, the objects msg, velPub, err_rollPub, err_pitchPub and err_yawPub are sent to "control_default" function.
PoseSub = rospy.Subscriber('/gazebo/model_states',ModelStates,control_default,(velPub, err_rollPub, err_pitchPub, err_yawPub))
